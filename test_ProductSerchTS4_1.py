...
# Opis przypadku testowego : Wyszukiwanie istniejącego towaru
...
class TestLoginNewUser:
    ...
    @pytest.mark.run(order=1)
    @pytest.mark.parametrize('nameProduct', [('Faded Short Sleeve T-shirts')])
    def test_EnterNameProductAndSearch(self,nameProduct):
        print(f' Uzycie wyszukiwarki po wpisaniu towaru')
        # given

        # nameProduct = 'Faded Short Sleeve T-shirts'
        ...
        searchproduct = TestBase.waitForElementXPATH_boolen(self.driver,
                                                           "//div[@class='right-block']//a[contains(text(),'Faded Short Sleeve T-shirts')]", 10)
        if searchproduct:
            error = True
        else:
            error = False
